        ...
        m_currentVariantCtrl( nullptr )
{
    ...
    m_agentPanel = new AI_AGENT_PANEL( this, AI_EDITOR_KIND::Schematic,
            [this]()
            {
                ...
                snapshot.m_ToolState.m_EditorKind = AI_EDITOR_KIND::Schematic;
                snapshot.m_ToolState.m_ContextVersion = snapshot.m_Version;
                snapshot.m_ToolState.m_Kind = snapshot.m_SelectedObjects.empty()
                                                       ? AI_TOOL_STATE_KIND::Idle
    ...
    if( GetToolManager() )
    {
        m_aiActivityObserverId = GetToolManager()->AddEventObserver(
                [this]( const TOOL_EVENT& aEvent )
                {
                    ...
                    auto record = MakeAiActivityRecordFromToolEvent( aEvent,
    ...
    m_auimgr.AddPane( m_agentPanel, EDA_PANE()
                      ...
                      .FloatingPosition( FromDIP( wxPoint( 65, 210 ) ) )
                      .Show( false ) );
    ...
    agentPane.Show( aui_cfg.show_agent_panel );
    ...
    if( aui_cfg.agent_panel_width > 0 )
        SetAuiPaneSize( m_auimgr, agentPane, aui_cfg.agent_panel_width, -1 );
...
void SCH_EDIT_FRAME::ToggleAgentPanel()
{
    ...
    EESCHEMA_SETTINGS* settings = eeconfig();
    wxAuiPaneInfo&     agentPaneInfo = m_auimgr.GetPane( AgentPaneName() );
    bool               show = !agentPaneInfo.IsShown();

    agentPaneInfo.Show( show );

    if( show )
    {
        SetAuiPaneSize( m_auimgr, agentPaneInfo, settings->m_AuiPanels.agent_panel_width, -1 );
    }
    else
    {
        settings->m_AuiPanels.agent_panel_width = m_agentPanel->GetSize().x;
        m_auimgr.Update();
    }
}


void SCH_EDIT_FRAME::ShowAgentModelSettings()
{
    if( m_agentPanel )

...
bool aiWindowIsOrContains( wxWindow* aParent, wxWindow* aCandidate )
{
    return aParent && aCandidate
           && ( aParent == aCandidate || aParent->IsDescendant( aCandidate ) );
}


bool aiEventTargetsWindow( const wxEvent& aEvent, wxWindow* aParent )
{
    return aiWindowIsOrContains( aParent,
                                 dynamic_cast<wxWindow*>( aEvent.GetEventObject() ) );
}


bool isAiPreviewPointerDownEvent( wxEventType aEventType )
{
    return aEventType == wxEVT_LEFT_DOWN || aEventType == wxEVT_RIGHT_DOWN
           || aEventType == wxEVT_MIDDLE_DOWN || aEventType == wxEVT_AUX1_DOWN
           || aEventType == wxEVT_AUX2_DOWN;
}
        ...
        m_eventCounterTimer( nullptr )
{
    ...
    m_agentPanel->ConfigureSuggestionReview(
            [this]( AI_AGENT_PANEL_MODEL& aModel, uint64_t aSuggestionId )
            {
                ...
                std::optional<VECTOR2I> delta =
                        ParseAiSuggestionMoveDelta( suggestion->m_ArgumentsJson );
            ...
            [this]( AI_AGENT_PANEL_MODEL& aModel, uint64_t aSuggestionId )
            {
                const bool rejected = aModel.RejectSuggestion( aSuggestionId );
    ...
    m_auimgr.AddPane( m_agentPanel, EDA_PANE().Name( AgentPaneName() )
    ...
    if( aui_cfg.agent_panel_width > 0 && m_agentPanel )
    {
        wxAuiPaneInfo& agentPanel = m_auimgr.GetPane( AgentPaneName() );
        SetAuiPaneSize( m_auimgr, agentPanel, aui_cfg.agent_panel_width, -1 );
    }
...
bool PCB_EDIT_FRAME::TryBefore( wxEvent& aEvent )
{
    if( m_agentPanel )
    {
        const wxEventType eventType = aEvent.GetEventType();

        if( eventType == wxEVT_CHAR || eventType == wxEVT_CHAR_HOOK )
        {
            wxKeyEvent& keyEvent = static_cast<wxKeyEvent&>( aEvent );
            const bool  hasModifier = keyEvent.GetModifiers() != wxMOD_NONE;
            const bool  focusInsideAgentPanel =
                    aiWindowIsOrContains( m_agentPanel, wxWindow::FindFocus() );

            if( m_agentPanel->HandlePreviewShortcut( keyEvent.GetKeyCode(), hasModifier,
                                                     focusInsideAgentPanel ) )
            {
                return true;
            }
        }
        else if( isAiPreviewPointerDownEvent( eventType )
                 && m_agentPanel->HandlePreviewPointer(
                         aiEventTargetsWindow( aEvent, m_agentPanel ) ) )
        {
            return true;
        }
    }

    return PCB_BASE_EDIT_FRAME::TryBefore( aEvent );
}
...
void PCB_EDIT_FRAME::SaveSettings( APP_SETTINGS_BASE* aCfg )
{
    ...
    if( cfg )
    {
        ...
        wxAuiPaneInfo& agentPane = m_auimgr.GetPane( AgentPaneName() );
...
bool PCB_EDIT_FRAME::AgentPaneShown()
{
    return m_auimgr.GetPane( AgentPaneName() ).IsShown();
}

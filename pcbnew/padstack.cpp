...
void EnsurePadstackEnumMaps()
{
    ENUM_MAP<PAD_DRILL_POST_MACHINING_MODE>& postMachiningMap =
            ENUM_MAP<PAD_DRILL_POST_MACHINING_MODE>::Instance();

    if( postMachiningMap.Choices().GetCount() == 0 )
    {
        postMachiningMap.Undefined( PAD_DRILL_POST_MACHINING_MODE::UNKNOWN )
    ...
    ENUM_MAP<BACKDRILL_MODE>& backdrillMap = ENUM_MAP<BACKDRILL_MODE>::Instance();

    if( backdrillMap.Choices().GetCount() == 0 )
    {
        backdrillMap.Undefined( BACKDRILL_MODE::NO_BACKDRILL )

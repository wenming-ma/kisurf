...
- Replay evaluation 已开始显式跟踪 rollback attempt：单条 trace 和 batch summary 都会输出 `rollback_attempt_count` 与 `rolled_back_attempt_count`。这用于衡量模型在 hidden attempt 中是否通过 rollback/retry 机制修正候选，而不是只看最终 published 状态。

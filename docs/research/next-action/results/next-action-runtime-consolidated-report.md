...
- Structured Surface work-state packet 已开始暴露 typed observation contract：`normalized_schema` 会把 `columns` / `schema.fields` / `tables[].columns` 归一化为字段列表和表结构，`field_origin_facts` 会把 `value_provenance` 归一化为可审计的 field-level origin facts。当前数据源仍是 panel state JSON，后续真实 Surface Adapter 需要覆盖 KiCad table / property panel / dialog / rule form 的数据流，并把 patch mechanics lowering 到 SurfacePatch。

当前最重要的真实缺口：

- Observation packet 已有 bounded context snapshot、dependency fingerprint 和 MVP `work_state_packet`；Placement / Routing 已开始暴露 planning target、结构化 anchor records、visible object summaries、routing active segment geometry、candidate landing facts，以及 placement obstacle / keepout / footprint geometry facts，Structured Surface 已开始暴露 schema / target scope / neighboring values / value provenance / validation state、normalized schema 和 field origin facts。但仍缺少生产级细节：native PCB adapter 对真实 obstacle / keepout / footprint / courtyard / board-outline / clearance source 的深度采集，以及 structured surface 的真实 adapter 和 SurfacePatch lowering。

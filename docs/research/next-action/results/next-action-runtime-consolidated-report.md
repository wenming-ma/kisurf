...
- 当前 attempt limit 已变成 work-state-aware policy，并进入 observation；同 fingerprint reject 后的重复发布已被静默抑制。policy 已开始吸收 runtime history signal：同 work-state 的 rejected preview 会形成 `reject_history_count`，superseded / expired preview 会形成 `context_churn_count`，两者都会作为 `attempt_policy.adjustments[]` 暴露给模型并收紧后续未显式选中候选时的 attempt loop。仍未完成的是按 latency、候选质量、work-state risk、更长期 ignore history 等更细信号动态调整预算。

...
- 旧 `AI_SUGGESTION_ORCHESTRATOR` 已从可构建源码中删除：suggestion queue / preview / accept / expire 的主路径只保留在 `AI_NEXT_ACTION_RUNTIME`。仍保留的 `AI_SUGGESTION_PROVIDER` 只是 model-backed suggestion parser 的窄接口，不再拥有发布或生命周期决策权。

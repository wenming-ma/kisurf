...
- Observation packet 已有 bounded context snapshot、dependency fingerprint 和 MVP `work_state_packet`；Placement / Routing 已开始暴露 planning target、结构化 anchor records、visible object summaries 和 routing active segment geometry，Structured Surface 已开始暴露 schema / target scope / neighboring values / value provenance / validation state。但仍缺少生产级细节：candidate landing facts、placement obstacle / keepout / footprint geometry facts，以及 structured surface 的真实 adapter、typed schema normalization、field origin tracking 和 SurfacePatch lowering。

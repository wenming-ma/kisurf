...
- Observation packet 已有 bounded context snapshot、dependency fingerprint 和 MVP `work_state_packet`；Placement / Routing 已开始暴露 planning target、结构化 anchor records、visible object summaries、routing active segment geometry、candidate landing facts，以及 placement obstacle / keepout / footprint geometry facts，Structured Surface 已开始暴露 schema / target scope / neighboring values / value provenance / validation state、normalized schema、field origin facts、hidden `SurfacePatch` lowering、render-level `surface_patch_previews` / normalized diff entries 和 common JSON-backed accept replay。Native PCB adapter 已开始补真实 board outline、footprint bbox / pads bbox / courtyard bbox、基础 clearance source、board-level constraint facts、board-level obstacle facts、per-net netclass sizing facts、bounded per-net topology graph facts、enabled / visible / active layer context、connectivity / ratsnest summary 和 bounded unconnected edge geometry sample。仍缺少生产级细节：native PCB adapter 对 viewport/locality-aware obstacles、custom-rule effective clearance / viewport-derived geometry / complete connectivity graph reconstruction 的深度采集，以及真实 KiCad structured surface adapter、真实 SurfacePatch visual diff、surface revision gate 和 KiCad UI/undo/dirty-state commit。

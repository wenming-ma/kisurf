...
- 更新：Preview Gate 已开始消费 validation 子事实中的 `rule_load` / `connectivity` / `refill` 阻断标记；native DRC 即使整体返回 `validated`，只要规则加载或派生状态明确 `blocks_publish`，runtime 也不会把结果 promotion 成用户可见 preview。

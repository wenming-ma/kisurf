...
- Placement work-state packet 已开始暴露生产 observation contract 的几何事实：`placement_obstacle_facts` 从 visible object details 归一化 via / footprint obstacle，`placement_keepout_facts` 暴露 keepout/no-placement 区域，`placement_footprint_geometry_facts` 暴露 footprint bbox / courtyard / pads geometry。Native PCB adapter 已开始把真实 footprint geometry 接入同一 contract：footprint details 会携带来自 KiCad geometry API 的 `bbox`、`pads_bbox` 和 `courtyard_bbox`，并且 board summary 已有 bounded `obstacle_facts`、DRC-engine-backed effective constraint facts、pair-specific clearance evaluation sample 和 component-level connectivity graph facts，供 Placement observation 和 self-review 使用。当前仍需继续补 geometry-specific effective rule resolution、viewport-derived geometry 和更深的 complete connectivity graph reconstruction。
...
- Observation packet 已有 bounded context snapshot、dependency fingerprint 和 MVP `work_state_packet`；Placement / Routing 已开始暴露 planning target、结构化 anchor records、visible object summaries、routing active segment geometry、candidate landing facts，以及 placement obstacle / keepout / footprint geometry facts，Structured Surface 已开始暴露 schema / target scope / neighboring values / value provenance / validation state、normalized schema、field origin facts、hidden `SurfacePatch` lowering、render-level `surface_patch_previews` / normalized diff entries 和 common JSON-backed accept replay。Native PCB adapter 已开始补真实 board outline、footprint bbox / pads bbox / courtyard bbox、基础 clearance source、board-level constraint facts（含 DRC engine worst constraints 和 pair-specific effective clearance sample）、board-level obstacle facts、per-net netclass sizing facts、bounded per-net topology graph facts（含 component-level graph reconstruction）、enabled / visible / active layer context、connectivity / ratsnest summary 和 bounded unconnected edge geometry sample。仍缺少生产级细节：native PCB adapter 对 viewport/locality-aware obstacles、geometry-specific effective rule resolution、viewport-derived geometry / deeper complete connectivity graph reconstruction 的深度采集，以及真实 KiCad structured surface adapter、真实 SurfacePatch visual diff、surface revision gate 和 KiCad UI/undo/dirty-state commit。

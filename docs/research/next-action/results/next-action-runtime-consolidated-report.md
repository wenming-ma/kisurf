...
- Replay evaluation 已开始显式跟踪 Preview Gate feedback：单条 trace 和 batch summary 都会输出 `preview_gate_feedback_count`。这用于衡量 `render_freshness_failed` / `validation_freshness_failed` / 可修复 gate failure 是否真的回灌进模型 review loop，而不是直接终止或被最终 publish 状态掩盖。

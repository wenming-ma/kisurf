...
- Placement work-state packet 已开始暴露生产 observation contract 的几何事实：`placement_obstacle_facts` 从 visible object details 归一化 via / footprint obstacle，`placement_keepout_facts` 暴露 keepout/no-placement 区域，`placement_footprint_geometry_facts` 暴露 footprint bbox / courtyard / pads geometry。Native PCB adapter 已开始把真实 footprint geometry 接入同一 contract：footprint details 会携带来自 KiCad geometry API 的 `bbox`、`pads_bbox` 和 `courtyard_bbox`，供 Placement observation 和 self-review 使用。当前仍需继续补 layer visibility、connectivity/ratsnest、effective clearance resolution 和更细的 keepout/constraint facts。
- Native PCB context adapter 已开始输出 board-level observation facts：`AI_CONTEXT_SNAPSHOT.m_Summary` 会携带 `pcb_board_summary` JSON，包含真实 net / footprint / pad / track / arc / via / drawing / Edge.Cuts / zone / keepout 数量、来自 `BOARD::GetBoardEdgesBoundingBox()` 的 board edge bbox，以及 `BOARD_DESIGN_SETTINGS` / default netclass 暴露的基础 clearance source facts。这样 Next Action observation 不再只能依赖 visible object 列表来推断工作范围和基础规则来源。
...
- Observation packet 已有 bounded context snapshot、dependency fingerprint 和 MVP `work_state_packet`；Placement / Routing 已开始暴露 planning target、结构化 anchor records、visible object summaries、routing active segment geometry、candidate landing facts，以及 placement obstacle / keepout / footprint geometry facts，Structured Surface 已开始暴露 schema / target scope / neighboring values / value provenance / validation state、normalized schema、field origin facts、hidden `SurfacePatch` lowering、render-level `surface_patch_previews` / normalized diff entries 和 common JSON-backed accept replay。Native PCB adapter 已开始补真实 board outline、footprint bbox / pads bbox / courtyard bbox 和基础 clearance source。仍缺少生产级细节：native PCB adapter 对真实 obstacle / keepout / layer visibility / connectivity / ratsnest / effective clearance 的深度采集，以及真实 KiCad structured surface adapter、真实 SurfacePatch visual diff、surface revision gate 和 KiCad UI/undo/dirty-state commit。

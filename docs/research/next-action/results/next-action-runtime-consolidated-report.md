...
- Preview Gate 已开始支持可修复失败的 review-loop 回灌：当模型请求 publish 但缺少 fresh render / fresh validation、没有满足 render / validation hint，或显式声明 `validation_gate_failed` 属于 `repairable_gate_failures[]` 时，runtime 会把结构化 `preview_gate_result` 作为内部 `preview_gate_feedback` tool result 注入同一个 review trace，让 provider 在预算内继续补证据；context drift、budget、semantic relevance、target scope mismatch 等终止型失败不会重入。
...
- 当前 attempt limit 已变成 work-state-aware policy，并进入 observation；同 fingerprint reject 后的重复发布已被静默抑制。policy 已开始吸收 runtime history signal：同 work-state 的 rejected preview 会形成 `reject_history_count`，superseded / expired preview 会形成 `context_churn_count`，超过 work-state wall-time budget 的 prior attempt 会形成 `latency_over_budget_count`，这些信号都会作为 `attempt_policy.adjustments[]` 暴露给模型并收紧后续未显式选中候选时的 attempt loop。仍未完成的是按候选质量、work-state risk、更长期 ignore history 等更细信号动态调整预算。

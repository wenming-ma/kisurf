...
- Placement / Routing work-state packet 已开始暴露生产 observation contract 的几何事实：`placement_obstacle_facts` 从 visible object details 归一化 via / footprint obstacle，`placement_keepout_facts` 暴露 keepout/no-placement 区域，`placement_footprint_geometry_facts` 暴露 footprint bbox / courtyard / pads geometry，并且会把 `cursor_region` / `viewport` 派生为 `locality_region` 与 `local_obstacle_facts`，让模型优先看到当前操作区域附近的障碍物；Routing 的 `local_obstacle_facts` 已从 visible track / arc / via details 推导局部 routing obstacle。Native PCB adapter 已开始把真实 footprint geometry 接入同一 contract：footprint details 会携带来自 KiCad geometry API 的 `bbox`、`pads_bbox` 和 `courtyard_bbox`，board summary 已有 bounded `obstacle_facts`、DRC-engine-backed effective constraint facts、pair-specific clearance evaluation sample、geometry-dependent rule presence metadata、pair EvalRules evaluation source、pair item bbox、per-net component topology 和 board-level `net_component_summaries`，供 Placement / Routing observation 和 self-review 使用。当前仍需继续补更深的 geometry-specific rule coverage、viewport-derived geometry 和更完整的 connectivity graph reconstruction。

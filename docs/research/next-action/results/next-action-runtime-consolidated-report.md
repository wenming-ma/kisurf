...
- Observation packet 已有 bounded context snapshot、dependency fingerprint 和 MVP `work_state_packet`；Placement / Routing 已开始暴露 planning target、结构化 anchor records、visible object summaries、routing active segment geometry、candidate landing facts，以及 placement obstacle / keepout / footprint geometry facts，Structured Surface 已开始暴露 schema / target scope / neighboring values / value provenance / validation state、normalized schema、field origin facts、hidden `SurfacePatch` lowering 和 render-level `surface_patch_previews`。但仍缺少生产级细节：native PCB adapter 对真实 obstacle / keepout / footprint / courtyard / board-outline / clearance source 的深度采集，以及 structured surface 的真实 adapter、真实 SurfacePatch visual diff 和 accept replay。
...
- SurfacePatch 如何映射到 KiCad undo/redo、dialog Apply/OK、project file dirty state。

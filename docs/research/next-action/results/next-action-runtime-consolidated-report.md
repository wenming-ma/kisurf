...
- 更新：observation contract 和 native PCB adapter 已开始提供 board-level flat `component_graph_nodes` / `component_graph_edges`；adapter 会把每个 net 的 connected copper island 输出为 graph node，并把 ratsnest component edge 输出为 graph edge，保留 net、component id、bbox、item labels、endpoint geometry、visible 状态和 endpoint-derived `estimated_manhattan_length`；runtime 已把 active net graph edges 投影为带 deterministic `priority_rank` 的 `routing_reachability_facts`。当前仍需要继续补更细 specialized routing graph facts，例如 layer-specific reachability、更完整 routed/remaining length 和真实 router cost。

...
- Observation packet 已有 bounded context snapshot、dependency fingerprint 和 MVP `work_state_packet`；Placement / Routing 已开始暴露 planning target、结构化 anchor records、visible object summaries、routing active segment geometry、candidate landing facts、基于 cursor-region / viewport 的 locality region 与 local obstacle facts（Routing 已覆盖 track / arc / via / pad 局部障碍），以及 placement obstacle / keepout / footprint geometry facts，Structured Surface 已开始暴露 schema / target scope / neighboring values / value provenance / validation state、normalized schema、field origin facts、hidden `SurfacePatch` lowering、render-level `surface_patch_previews` / normalized diff entries 和 common JSON-backed accept replay。Native PCB adapter 已开始补真实 board outline、footprint bbox / pads bbox / courtyard bbox、zone / keepout bbox、真实 editor viewport / cursor-region、基础 clearance source、board-level constraint facts（含 DRC engine worst constraints、pair-specific effective clearance sample、geometry-dependent rule presence、pair EvalRules source 和 pair item bbox）、board-level obstacle facts、per-net netclass sizing facts、bounded per-net topology graph facts（含 component-level graph reconstruction）、board-level per-net component summaries、enabled / visible / active layer context、connectivity / ratsnest summary 和 bounded unconnected edge geometry sample。仍缺少生产级细节：native PCB adapter 对更深 geometry-specific effective rule coverage、更完整 interactive anchor/corridor semantics 和 deeper complete connectivity graph reconstruction 的深度采集，以及真实 KiCad structured surface adapter、真实 SurfacePatch visual diff、surface revision gate 和 KiCad UI/undo/dirty-state commit。

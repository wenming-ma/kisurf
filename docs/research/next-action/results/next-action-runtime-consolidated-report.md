...
- Placement / Routing work-state packet 已开始暴露生产 observation contract 的几何事实：`placement_obstacle_facts` 从 visible object details 归一化 via / pad / footprint obstacle，`placement_keepout_facts` 暴露 keepout/no-placement 区域并兼容 native zone details 的 `zone_kind:"keepout"` / `has_keepout:true` 形式，`placement_footprint_geometry_facts` 暴露 footprint bbox / courtyard / pads geometry，并且会把 `cursor_region` / `viewport` 派生为 `locality_region` 与 `local_obstacle_facts`，让模型优先看到当前操作区域附近的障碍物；Routing 的 `local_obstacle_facts` 已从 visible track / arc / via / pad details 推导局部 routing obstacle，`routing_corridor_facts` 会把 route anchors 转换为从当前 route head 到候选落脚点的 bounded corridor facts，包含 start/end、delta、segment style、manhattan length、net/layer/width 和手动点击 materialize 语义。work-state packet 现在还会从 `AI_CONTEXT_SNAPSHOT.m_Summary` 注入 bounded `constraint_summary` 和 `connectivity_summary`，前者包含 minimums、rule / keepout count、DRC engine presence、geometry-dependent rule metadata、worst constraints 和 pair effective constraint sample，后者包含 ratsnest / node / pad counts、per-net component summary sample 和 unconnected edge sample，让 Placement / Routing 的 decision / review turn 可以直接看到当前板级约束和连通性，不再只靠 visible objects 推断规则和 airwire 状态。Native PCB adapter 已开始把真实 footprint / zone geometry 接入同一 contract：footprint details 会携带来自 KiCad geometry API 的 `bbox`、`pads_bbox` 和 `courtyard_bbox`，zone / keepout visible-object details 会携带 `bbox` 以支持 locality 过滤；PCB tool-state provider 已从真实 `KIGFX::VIEW` 输出 `viewport`，并从当前 tool cursor 输出可 fingerprint 的 `cursor_region`；board summary 已有 bounded `obstacle_facts`、DRC-engine-backed effective constraint facts、pair-specific clearance evaluation sample、geometry-dependent rule presence metadata、pair EvalRules evaluation source、pair item bbox、per-net component topology 和 board-level `net_component_summaries`，供 Placement / Routing observation 和 self-review 使用。当前仍需继续补更深的 geometry-specific rule coverage、更完整的交互 anchor/corridor 语义和 connectivity graph reconstruction。
...
- Observation packet 已有 bounded context snapshot、dependency fingerprint 和 MVP `work_state_packet`；Placement / Routing 已开始暴露 planning target、结构化 anchor records、visible object summaries、routing active segment geometry、routing corridor facts、candidate landing facts、基于 cursor-region / viewport 的 locality region 与 local obstacle facts（Routing 已覆盖 track / arc / via / pad 局部障碍）、placement obstacle / keepout / footprint geometry facts，以及从 board summary 派生的 bounded `constraint_summary` / `connectivity_summary`。Structured Surface 已开始暴露 schema / target scope / neighboring values / value provenance / validation state、normalized schema、field origin facts、surface guard facts、hidden `SurfacePatch` lowering、render-level `surface_patch_previews` / normalized value-aware diff entries / overlay `visual_target` / diff summary、preview-time `AI_PREVIEW_MANAGER` structured-surface overlay projection、wxGrid / wxPropertyGrid preview overlay seam、common JSON-backed accept replay / revision/schema/selection/overlap mismatch gate，以及 source-backed grid / wxGrid 和 field / wxPropertyGrid accept backend 第一刀。Native PCB adapter 已开始补真实 board outline、footprint bbox / pads bbox / courtyard bbox、zone / keepout bbox、真实 editor viewport / cursor-region、基础 clearance source、board-level constraint facts（含 DRC engine worst constraints、pair-specific effective clearance sample、geometry-dependent rule presence、pair EvalRules source 和 pair item bbox）、board-level obstacle facts、per-net netclass sizing facts、bounded per-net topology graph facts（含 component-level graph reconstruction）、board-level per-net component summaries、enabled / visible / active layer context、connectivity / ratsnest summary 和 bounded unconnected edge geometry sample。仍缺少生产级细节：native PCB adapter 对更深 geometry-specific effective rule coverage、更完整 interactive anchor/corridor semantics 和 deeper complete connectivity graph reconstruction 的深度采集，以及 dialog / rule form 等更复杂 KiCad structured surface adapter、真实 KiCad source-backed surface revision / schema / selection / overlap 来源和 KiCad UI/undo/dirty-state commit。

...
- 复合 structured surface 的底层 accept backend 已有 coordinated commit / abort substrate，并已收紧 child ownership / scoped replay 边界；剩余问题应收敛为真实 KiCad dialog/rule-form adapter 如何暴露 revision、schema、selection、overlap、dirty-state、OK/Apply 和 grouped undo，而不是继续停留在 JSON replay 是否可行。

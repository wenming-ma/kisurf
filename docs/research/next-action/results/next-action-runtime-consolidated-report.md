...
- 更新：decision turn 中由 integrated candidate tool 生成的候选已能被提升到 runtime candidate pool，并排在 work-state fallback candidate 之前。这样模型可以走 “调用候选工具 -> 选择 `selected_candidate_index` -> hidden attempt -> render/validate/review” 的闭环；当前只提升能解析成直接 `AI_SUGGESTION_OPERATION` 并可由 runtime 立即 attempt 的候选，计划型 / 事实型候选仍需通过 bounded script 或 repair wrapper 进入 hidden mutation batch。

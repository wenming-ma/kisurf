...
nlohmann::json placementObstacleFactJson( const AI_OBJECT_REF& aObject )
{
    ...
    if( aObject.m_Type == PCB_PAD_T || detailsKindEquals( details, "pad" ) )
    {
        ...
        if( !fact.contains( "footprint" ) && details.contains( "footprint_reference" ) )
            fact["footprint"] = details["footprint_reference"];

        if( !fact.contains( "footprint_reference" ) && details.contains( "footprint" ) )
            fact["footprint_reference"] = details["footprint"];

        if( !fact.contains( "pad_name" ) && details.contains( "number" ) )
            fact["pad_name"] = details["number"];

        if( !fact.contains( "pad_number" ) )
        {
            if( details.contains( "number" ) )
                fact["pad_number"] = details["number"];
            else if( details.contains( "pad_name" ) )
                fact["pad_number"] = details["pad_name"];
        }
    ...
    if( aObject.m_Type == PCB_FOOTPRINT_T || detailsKindEquals( details, "footprint" ) )
    {
        ...
        copyJsonFieldIfPresent( fact, details, "value" );
        copyJsonFieldIfPresent( fact, details, "bbox" );
...
AiEvaluateNextActionReplayGoldenRecordJson( const wxString& aGoldenRecordJson )
{
                ...
                if( aTraceEval )
                {
                    ...
                    summary["trace_preview_gate_feedback_reason_counts"] =
                            parseJsonObjectOrEmpty(
                                    aTraceEval->m_PreviewGateFeedbackReasonCountsJson );
    ...
    if( expected.contains( "min_preview_gate_feedback_reason_counts" )
        && expected["min_preview_gate_feedback_reason_counts"].is_object() )
    {
        nlohmann::json previewGateFeedbackReasonCounts =
                parseJsonObjectOrEmpty(
                        traceEval.m_PreviewGateFeedbackReasonCountsJson );

        for( auto it = expected["min_preview_gate_feedback_reason_counts"].begin();
             it != expected["min_preview_gate_feedback_reason_counts"].end(); ++it )
        {
            if( !( it.value().is_number_unsigned()
            ...
            if( jsonCounterValue( previewGateFeedbackReasonCounts, it.key() )
                < minimum )
            {
                return finish(
                        true, false,
                        wxS( "preview_gate_feedback_reason_count_below_minimum" ),
                        wxS( "Replay preview gate feedback reason count is below expected minimum." ),
                        &traceEval );
            }
        }
    }
...
AiEvaluateNextActionReplayGoldenDataset( const wxArrayString& aGoldenRecordJsons )
{
    ...
    nlohmann::json tracePreviewGateFeedbackReasonCounts =
            nlohmann::json::object();
    ...
    for( size_t i = 0; i < aGoldenRecordJsons.size(); ++i )
    {
        ...
        result.m_TracePreviewGateFeedbackCount +=
                jsonCounterValue( recordSummary,
                                  "trace_preview_gate_feedback_count" );
        ...
        if( recordSummary.contains(
                    "trace_preview_gate_feedback_reason_counts" ) )
        {
            mergeJsonCounters(
                    tracePreviewGateFeedbackReasonCounts,
                    recordSummary["trace_preview_gate_feedback_reason_counts"] );
        }
    ...
    result.m_TracePreviewGateFeedbackReasonCountsJson =
            fromUtf8String( tracePreviewGateFeedbackReasonCounts.dump() );
...
AiEvaluateNextActionReplayGoldenDatasetFiles( const wxArrayString& aGoldenDatasetPaths )
{
    ...
    for( size_t i = 0; i < aGoldenDatasetPaths.size(); ++i )
    {
        ...
        result.m_TracePreviewGateFeedbackCount +=
                dataset.m_TracePreviewGateFeedbackCount;
        result.m_TraceAcceptGateResultCount +=
        ...
        nlohmann::json datasetTracePreviewGateFeedbackReasonCounts =
                parseJsonObjectOrEmpty(
                        dataset.m_TracePreviewGateFeedbackReasonCountsJson );

        if( datasetTracePreviewGateFeedbackReasonCounts.empty()
            && datasetSummary.contains(
                    "trace_preview_gate_feedback_reason_counts" )
            && datasetSummary["trace_preview_gate_feedback_reason_counts"]
                       .is_object() )
        {
            datasetTracePreviewGateFeedbackReasonCounts =
                    datasetSummary["trace_preview_gate_feedback_reason_counts"];
        }

        mergeJsonCounters( tracePreviewGateFeedbackReasonCounts,
                           datasetTracePreviewGateFeedbackReasonCounts );
                            ...
                            return nlohmann::json{
                                { "pcb.create_via",
                                  { { "type", "object" },
                                    { "additionalProperties", true },
                                    { "properties",
                                      { { "position", pointSchema( "Via center position." ) },
                                        { "net", { { "type", "string" } } },
                                        { "diameter",
                                          { { "type", "integer" }, { "minimum", 1 } } },
                                        { "drill",
                                        ...
                                        { "layer_pair",
                                          { { "description",
                                              "Via layer pair, usually [start, end]." },
                                            { "anyOf",
                                              nlohmann::json::array(
                                        ...
                                        { "metadata",
                                          { { "type", "object" },
                                            { "additionalProperties", true } } } } },
                                    { "required",
                                      nlohmann::json::array( { "position" } ) } } },
                                { "pcb.create_track_segment",
                                  { { "type", "object" },
                                    { "additionalProperties", true },
                                    { "properties",
                                      { { "start", pointSchema( "Track segment start point." ) },
                                        { "end", pointSchema( "Track segment end point." ) },
                                        { "layer", { { "type", "string" } } },
                                        { "net", { { "type", "string" } } },
                                        { "width",
                                          { { "type", "integer" }, { "minimum", 1 } } },
                                      ...
                                      { { "shape_type",
                                        ...
                                        { "metadata",
                                          { { "type", "object" },
                                            { "additionalProperties", true } } } } },
                                    { "required",
                                      nlohmann::json::array( { "shape_type",
                                                               "geometry" } ) } } },
                                { "pcb.move_items",
                                  { { "type", "object" },
                                    { "additionalProperties", true },
                                    { "properties",
                                      { { "handles", handleArraySchema( "Items to move." ) },
                                        { "delta", pointSchema( "Movement delta." ) },
                                        { "target_positions",
                                          { { "description",
                                              "Single target point, ordered target array, or keyed target map." },
                                            { "anyOf",
                                              nlohmann::json::array(
                                                      { pointSchema( "Single target point." ),
                                                        { { "type", "array" },
                                                          { "items",
                                                            pointSchema( "Ordered target point." ) } },
                                                        { { "type", "object" },
                                                          { "additionalProperties", true } } } ) } } } } },
                                    { "required",
                                      nlohmann::json::array( { "handles" } ) } } },
                                { "pcb.delete_items",
                                  { { "type", "object" },
                                    { "additionalProperties", false },
                                    { "properties",
                                      { { "handles",
                                          handleArraySchema( "Items to delete." ) } } },
                                    { "required",
                                      nlohmann::json::array( { "handles" } ) } } },
                                  ...
                                  { { "type", "object" },
                                    ...
                                    { "properties",
                                      { { "handle", handleSchema() },
                                        { "net", { { "type", "string" } } } } },
                                    { "required",
                                      nlohmann::json::array( { "handle", "net" } ) } } },
                                { "pcb.set_item_layer",
                                  { { "type", "object" },
                                    { "additionalProperties", false },
                                    { "properties",
                                      { { "handle", handleSchema() },
                                        { "layer", { { "type", "string" } } },
                                        { "layer_set",
                                          stringArraySchema( "Layer set for multilayer items." ) } } },
                                    { "required",
                                      nlohmann::json::array( { "handle" } ) } } },
                                  ...
                                  { { "type", "object" },
                                    ...
                                    { "properties",
                                      { { "surface_id", { { "type", "string" } } },
                                        ...
                                        { "expected_surface_revision",
                                          { { "description",
                                              "Surface revision expected at accept time." } } },
                                        { "expected_schema_version",
                                          { { "description",
                                              "Surface schema version expected at accept time." } } },
                                        { "expected_selection_fingerprint",
                                          { { "description",
                                              "Focused selection fingerprint expected at accept time." } } },
                                        { "expected_overlap_set",
                                          { { "description",
                                              "Protected overlap set expected at accept time." } } },
                                        { "alias", { { "type", "string" } } },
                                        { "metadata",
                                          { { "type", "object" },
                                            { "additionalProperties", true } } } } },
                                    { "required",
                                      nlohmann::json::array( { "surface_id",
                                                               "patch" } ) } } }
                            };
                        };

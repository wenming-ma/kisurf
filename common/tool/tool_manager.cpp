...
uint64_t TOOL_MANAGER::AddEventObserver( EVENT_OBSERVER aObserver )
{
    const uint64_t id = m_nextEventObserverId++;
    m_eventObservers.emplace_back( id, std::move( aObserver ) );
    return id;
}


void TOOL_MANAGER::RemoveEventObserver( uint64_t aObserverId )
{
    m_eventObservers.erase(
            std::remove_if( m_eventObservers.begin(), m_eventObservers.end(),
                            [aObserverId]( const auto& aEntry )
                            {
                                return aEntry.first == aObserverId;
                            } ),
...
void TOOL_MANAGER::notifyEventObservers( const TOOL_EVENT& aEvent )
{
    const std::vector<std::pair<uint64_t, EVENT_OBSERVER>> observers = m_eventObservers;

    for( const auto& observerEntry : observers )
    {
        const EVENT_OBSERVER& observer = observerEntry.second;
